...
/**
 * Patient gender enum - REQUIRED per PocketBase schema
 */
export const patientGenderSchema = v.picklist(['male', 'female'], 'Please select a gender')

/**
 * Patient status enum - optional with default
 */
export const patientStatusSchema = v.optional(
  v.picklist(['active', 'inactive', 'archived']),
  'active'
)

/**
 * Complete patient form schema
 * Only firstName, lastName, and gender are REQUIRED (matches PocketBase schema)
 */
 ...
 */
export type PatientFormData = v.InferOutput<typeof patientFormSchema>

/**
 * Gender options for select dropdowns (matches PocketBase schema)
 */
