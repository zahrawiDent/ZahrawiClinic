import { Button, FormCard, Select, TextInput } from "@/components/forms";
import { toast } from "@/components/toast";
import { useAuth } from "@/lib/auth-context";
import { getCurrentUserCollection, isSuperuser } from "@/lib/auth-helpers";
...
function EditProfilePage() {
  ...
  const handleSubmit: SubmitHandler<
    typeof SuperuserFormSchema | typeof UsersFormSchema
  > = async (values) => {
    try {
              ...
              <Show when={!isSuper}>
                ...
                <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
                  <p class="text-sm text-amber-800 dark:text-amber-200">
                    <strong>Note:</strong> Role field is read-only. Contact an
